...
#endif
{
   ...
   // get the engine functions from the engine... //Removed support for Non-Steam - [APG]RoboCop[CL]
	...
	if (strcmpi(mod_name, "ice_v1_1") == 0)
	{
		mod_id = VALVE_DLL;
		         ...
		         if (access("ice_v1_1/dlls/ice.dll", 0) == -1)
		         {
			         filebuf = LOAD_FILE_FOR_ME ("dlls/ice.dll", &filesize);
			         if (filebuf != NULL)
			         {
				         filep = fopen ("ice_v1_1/dlls/ice.dll", "wb");
				         if (filep != NULL)
				         {
		         ...
		         h_Library = LoadLibrary("ice_v1_1/dlls/ice.dll"); // and load the library
         #else
		         // test if the game DLL file is NOT available outside of the Steam/PAK cache
		         if (access("ice_v1_1/dlls/ice.so", 0) == -1)
		         {
			         filebuf = LOAD_FILE_FOR_ME ("dlls/ice.so", &filesize);
			         if (filebuf != NULL)
			         {
				         filep = fopen ("ice_v1_1/dlls/ice.so", "wb");
				         if (filep != NULL)
				         {
					         fwrite (filebuf, 1, filesize, filep); // if in cache, then extract it
					         fclose (filep);
		         ...
		         h_Library = dlopen("ice_v1_1/dlls/ice.so", RTLD_NOW);
         #endif
      #endif
	}
