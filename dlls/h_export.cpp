...
#endif
{
   ...
   // get the engine functions from the engine... //Removed support for Non-Steam - [APG]RoboCop[CL]
	...
	if (strcmpi(mod_name, ICE_MOD_NAME) == 0)
	{
		mod_id = VALVE_DLL;
		         ...
		         if (access(ICE_MOD_NAME "/dlls/ice.dll", 0) == -1)
		         {
			         filebuf = LOAD_FILE_FOR_ME ("dlls/ice.dll", &filesize);
			         if (filebuf != NULL)
			         {
				         filep = fopen (ICE_MOD_NAME "/dlls/ice.dll", "wb");
				         if (filep != NULL)
				         {
		         ...
		         h_Library = LoadLibrary(ICE_MOD_NAME "/dlls/ice.dll"); // and load the library
         #else
		         // test if the game DLL file is NOT available outside of the Steam/PAK cache
		         if (access(ICE_MOD_NAME "/dlls/ice.so", 0) == -1)
		         {
			         filebuf = LOAD_FILE_FOR_ME ("dlls/ice.so", &filesize);
			         if (filebuf != NULL)
			         {
				         filep = fopen (ICE_MOD_NAME "/dlls/ice.so", "wb");
				         if (filep != NULL)
				         {
					         fwrite (filebuf, 1, filesize, filep); // if in cache, then extract it
					         fclose (filep);
		         ...
		         h_Library = dlopen(ICE_MOD_NAME "/dlls/ice.so", RTLD_NOW);
         #endif
      #endif
	}
